...
covid_data = covid_data.drop(['county', 'deaths', 'state', 'fips'], axis=1)

# undo accumulation
covid_data['cases'] -= covid_data['cases'].shift(1).fillna(0)
covid_data['cases'] = covid_data['cases'].astype(int)
covid_data = covid_data.set_index('date')
covid_data = covid_data.clip(0, covid_data['cases'].max())
...
# define a LSTM (predicting cases)
model = Sequential()
model.add(LSTM(units=n_lag, activation='relu', input_shape=(n_lag, 1), return_sequences=True))
model.add(Dropout(0.2))
model.add(LSTM(units=30, activation='relu'))
model.add(Dropout(0.2))
model.add(Dense(units=1, activation='sigmoid'))

# compile on the data (COVID-19 Cases in Pierce County)
...
plt.plot(covid_data.index, covid_data['cases'], label='actual')
plt.title('COVID-19 Cases in Pierce County')
plt.xlabel('Time')
plt.ylabel('Cases')
plt.legend()
plt.show()
